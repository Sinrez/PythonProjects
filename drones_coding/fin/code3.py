...
#Флаг для управления бесконечным полетом
is_infinity_flying = False
...
def create_telemetry_csv():
    """создание файла для телеметирии в файл csv"""
...
def show_telemetry():
    """Отображение телеметрии дрона"""
    telem = get_telemetry()
    status_label.config(text=(
        f"Телеметрия:\n"
        f"Широта: {telem.lat} Долгота: {telem.lon}\n"
        f"Pitch: {round(telem.pitch, 6)} Armed: {telem.armed}\n"
    ))
...
def land_drone():
    """Приземление дрона"""
    status_label.config(text="Дрон приземляется...")
    threading.Thread(target=land).start()
    rospy.sleep(2)  # Даем время для приземления
    status_label.config(text="Дрон приземлился!")

def infinity_fly():
    """Бесконечный полет дрона по случайным координатам"""
    global is_infinity_flying
    print('Запущен режим бесконечного автономного полета')
    while is_infinity_flying:  # Проверка флага для остановки
        x = random.uniform(-10, 10)
        y = random.uniform(-10, 10)
        print(f"Дрон летит в точку: X={x}, Y={y}")
        status_label.config(f"Дрон летит в точку: X={x}, Y={y}")
        # navigate(x=x, y=y, z=3, frame_id='body', speed=1)
        threading.Thread(target=navigate, args=(x, y, 3, 1, "body", False)).start()
        arrival_wait()
        status_label.config(text=f"Дрон достиг точки Х: {x}, Y: {y} с высотой Z: {z} и скоростью {speed}")

def stop_infinity_fly():
    """Остановка бесконечного полета"""
    global is_infinity_flying
    is_infinity_flying = False
    status_label.config(text='Бесконечный полет остановлен')
    print('Бесконечный полет остановлен')

def fly_to_local_coordinates():
    """Функция полета по локальным координатам"""
    global is_infinity_flying
    is_infinity_flying = False #для остановки полета по случайным координатам, если включен
    global z
    global speed
    try:
        if entry_x.get():
...
def fly_to_global_coordinates():
    """Функция полета по глобальным координатам"""
    global is_infinity_flying
    is_infinity_flying = False #для остановки полета по случайным координатам, если включен
    try:
...
def browse_files():
    """Проверка загрузки плана полета"""
...
# Распознание человека с каскадом
def detect_objects():
    global latest_image
...
infinity_fly_button = tk.Button(window, text="Бесконечный полет", width=20, relief="solid", activebackground="silver", command=lambda: infinity_fly())
infinity_fly_button.grid(row=8, column=1, padx=20, pady=10)

# Кнопка для остановки бесконечного полета
stop_infinity_fly_button = tk.Button(window, text="Остановить полет", width=20, relief="solid", activebackground="silver", command=stop_infinity_fly)
stop_infinity_fly_button.grid(row=8, column=2, padx=20, pady=10)
